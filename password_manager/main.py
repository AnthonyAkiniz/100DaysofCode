...
#########################################################################################
# Info:                                                                                 #
# Helps to manage passwords for various websites.                                       #
#                                                                                       #
# Guide:                                                                                #
# cd password_manager                                                                   #
...
def save():
    ...
    if len(website) < 1 or len(email) < 1 or len(password) < 1:
        ...
        response = messagebox.askokcancel(
            title="Please confirm", message=f"Website: {website}\nEmail: {email}\nPassword: {password}\n\nOK to save?")
        if response:
            try:
...
def search():
    ...
    try:
        ...
        if website in file:
            ...
            # Pop-up messagebox
            messagebox.showinfo(
                title=website, message=f"Website {website} not found in file 'data.json'")
        # Delete the entries in the GUI
        entry_website.delete(0, tk.END)
        entry_password.delete(0, tk.END)
...
# Inserts string just before the character indicated by index.
